...
module.exports = class Metalsmith {
  ...
  async build(){
    if (this.clean()) {
      try       { await fs.remove(this.destination()) } 
      catch (e) { throw e } 
    }
  ...
  async run(files, plugins){
    ...
    const run = util.promisify(ware.run.bind(ware))
    const res = await run(files, this)
    return res
  }
  ...
  async read(dir){
    ...
    const paths = await readdir(dir, ignores)
    let files = []
    
    for (let complete=0 ; complete < paths.length ; complete += concurrency) {
      const batch = paths.slice(complete, complete + concurrency)
      const readBatch = await Promise.all(batch.map(readFile))
      files = files.concat(readBatch)
    }
  ...
  async write(files, dir){
    ...
    for (let complete=0 ; complete < keys.length ; complete += concurrency) {
      const batch = keys.slice(complete, complete + concurrency)
      await Promise.all(batch.map(writer))
    }

    function writer(key){
      const file = path.resolve(dir, key)
      return writeFile(file, files[key])
    }
  }
